...
def my_detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45, debug=False):
    #pylint: disable= C0321
    if False:
        im = darknet.load_image(image.encode('ascii'), 0, 0)
    else:
        import cv2
        custom_image = cv2.imread(image)
        im, arr = darknet.array_to_image(custom_image)
    if debug:
        print("Loaded image")
    det = my_detect_(net, meta, im, thresh, hier_thresh, nms, debug)
    if False:
...
def my_detect_(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45, debug=False):
    """
...
r = my_detect(net, meta, to_str("img/air_vapor/air_vapor_original_shot_2018-May-07-22-12-34-237301_3fd9a907-034f-45c0-9a84-f4431945fa7b.jpg"), thresh=0.25, debug=False)
print(r)
